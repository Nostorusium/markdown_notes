...
int* p = new int[5];
```
如果在堆上分配,获取到指针p,sizeof(p)只能得到一个int*指针的大小而不是该指针所代表的数组长度,即编译器只能知道p是一个指向int的指针而不知道其具体内容,也无从判断长度
...
```
之所以不能改变是因为不同于栈上分配声明了大小的数组,这种指针仅仅指向一个固定分配的内存块,你无法延长或缩短这一块.
如果你想要一个更长的字符串那就分配一个全新的并删除旧的
这个char*不在堆上分配 也不能delete(因为你没有new)
字符串是不可变的,所以通常用const char*定义.
